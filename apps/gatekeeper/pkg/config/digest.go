...
// (one trailing `=` tolerated), `sha256/<64 hex>`, bare hex, and `sha256:<64
// hex>` — the form the CLI prints, the console copies and this file is written
// with, and the one a user is most likely to paste.
//
...
func ParseEvidenceDigest(value string) (string, error) {
	return attestation.NormalizeEvidenceDigest(value)
}
...
func shapeErrorf(node *yaml.Node, format string, args ...any) error {
	return fmt.Errorf("line %d: %s (write one pin per line: `- sha256:<64 hex chars>`)",
