 ...
 * the console's analytics ingest and the OpenAI-compatible `/v1`.
 *
 ...
 * module owning a named `/v1` path has to come first. `invites.e2e.spec.ts` and
 * `analytics.e2e.spec.ts` assert the resulting behaviour rather than this line.
 */
@Module({
  imports: [HealthModule, InvitesModule, AnalyticsIngestModule, V1Module],
})
